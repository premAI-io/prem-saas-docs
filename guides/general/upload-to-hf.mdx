...
Prem Studio offers two fine-tuning methods: LoRA and Full fine-tuning. If you don't know what LoRA is, [you can check that out here quickly](/guides/lora-finetuning). Here are some key differences:

- **LoRA fine-tuning** downloads only the additional adapter weights that were trained on top of the base model.
- **Full fine-tuning** downloads the complete model with all updated parameters. Note that these can be very large files.
