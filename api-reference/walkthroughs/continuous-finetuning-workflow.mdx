...
description: "Use traces to continuously improve your models by evaluating responses and iteratively fine-tuning."
---

## Overview

Continuously improve your fine-tuned model by:
- Testing with domain-specific prompts
- Evaluating responses with a judge model (Claude 4.5 Sonnet)
- Creating traces with scores and feedback
- Adding all traces to your dataset (low-scoring traces are automatically improved)
- Launching new fine-tuning jobs that learn from both successes and corrected failures
...
<Steps>
  ...
  Fetch project details to get the dataset ID. See [Get Project](/api-reference/projects/get-projects-projectid) for details.
  </Step>

  <Step>
  # Test model with prompts

  Generate responses from your fine-tuned model using test prompts. See [Chat Completions](/api-reference/chat-completions/post-chatcompletions) for details.
  ...
  Use a judge model to evaluate response quality. See [Chat Completions](/api-reference/chat-completions/post-chatcompletions) for details.
...
{{
  ...
  Create traces with evaluation scores and feedback. See [Create Trace](/api-reference/traces/post-traces) for details.
  ...
  Add traces to your dataset. Low-scoring traces are automatically improved using judge feedback. See [Add Trace to Dataset](/api-reference/traces/post-trace-add-to-dataset) for details.
  </Step>

  <Step>
  # Create snapshot

  Create a snapshot with train/validation split. See [Create Snapshot](/api-reference/snapshots/post-snapshots-create) for details.
  ...
  Generate and retrieve model recommendations for your snapshot. See [Generate Recommendations](/api-reference/recommendations/post-recommendations-generate) and [Get Recommendations](/api-reference/recommendations/get-recommendations-snapshotid) for details.
...
1. Requires `PROJECT_ID` and `FINETUNED_MODEL_ALIAS` from a previous workflow. `DATASET_ID` is auto-fetched.
2. Uses Claude 4.5 Sonnet to score responses (0-1 scale).
3. Add ALL traces to the dataset - low-scoring traces are automatically improved.
4. Each cycle adds training data focusing on model weaknesses.
