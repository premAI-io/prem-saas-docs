...
The objective of this recipe is to show developers and users on how to get started with Prem's Generative AI Platform. The overall tutorial is going to be done in four simple steps:
...
3. Writing a simple streamlit chat function that will take user prompt, get the context from Prem repositories and then pass it to the LLM to provide us accurate results.

After completion of this tutorial, you will be well familiar with Prem SDK and some of it's terms. You can checkout our other recipes which covers more intermediate and advanced GenAI usecases. 
...
With Prem Repositories, you can upload any kind of documents (like .txt, .pdf etc) and we take care of the rest. The workflow of our recipe is simple: we start by uploading some documents to a repository on-prem. Once uploaded, the repository will automatically index them using our internal state-of-the-art RAG technology. You can checkout more about [Prem repositories here](/get-started/repositories.mdx).
