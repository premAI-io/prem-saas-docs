...
Welcome to the Prem AI cookbook section. In this cookbook, we will explore how we can use [PremAI SDK](/get-started/sdk.mdx) and [DSPy](https://github.com/stanfordnlp/dspy) to generate SQL from text. This example shows one of the ways we can extend development with Prem AI. So without further ado, let’s get started. You can find the full code [here](https://github.com/premAI-io/cookbook/tree/main/text-2-sql).

## Objective

The objective of this tutorial is to introduce you to DSPy and show you how to use it with the Prem SDK. Finally, in the later part of the tutorial, we will demonstrate how to use the Prem Platform to debug, see what DSPy has been optimizing, and how it can be further improved. In this tutorial we are going to:
...
Most LLM orchestration frameworks, like LangChain and llama-index, advise you to prompt the language models explicitly. Eventually, we need to tweak or optimize the prompts several times to achieve better results. However, writing and managing large prompts can be quite messy.
...
Now we know how signatures work in DSPy. Multiple signatures or prompting technique could work as a function. You can compose those techniques into a single module or a program. 

If you come from a deep learning background, then you have heard about `torch.nn.Module`, which helps to compose multiple layers to a single program. Similarly, in DSPy, multiple modules can be composed into bigger modules (programs). Now, let's create a simple module for our Text2SQL generator.
...
<Warning>
It is important to note that, sometimes metrics can be very unreliable. For our example, a metric like `compare_sqls` is comparing two sql statement through string manipulation. However this is not the optimal approach to conclude the quality of the LLM. A better metric is to run the generated SQL statements inside a table of a database and see whether the results are matching with the ground truth or not. However that is out of the scope of this tutorial.
</Warning>

## Optimizing a DSPy program

An optimizer (previously knowsn as teleprompter) in DSPy optimizes the overall prompt workflow by tuning the prompt and/or the LM weights to maximize the target metrics, such as accuracy.

Optimizers take three things in the input:

1. **The DSPy Program**: This may be a single module (e.g., dspy.Predict) or a complex multi-module program. Already defined above.

2. **Metric Function:** This is the function that evaluates the program and assigns a score at the end. Already defined above.
